...
describe('Guide.vue', () => {
    ...
    it('First test button works', async () => {
        ...
        await wrapper.vm.$nextTick();
        expect(wrapper.vm.$route.fullPath).toBe("/results?includeDatasetResponses=HIT&assemblyId=GRCh38&referenceName=1&start=104431389&referenceBases=C&variantType=INS") 
        wrapper.destroy()   
    })
    it('Second test button works', async () => {
        ...
        await wrapper.vm.$nextTick();
        expect(wrapper.vm.$route.fullPath).toBe("/results?includeDatasetResponses=HIT&assemblyId=GRCh38&referenceName=MT&start=7599&referenceBases=G&alternateBases=A") 
        wrapper.destroy()   
    })
    it('Third test button works', async () => {
        ...
        await wrapper.vm.$nextTick();
        expect(wrapper.vm.$route.fullPath).toBe("/results?includeDatasetResponses=HIT&assemblyId=GRCh38&referenceName=MT&start=194&referenceBases=TTACTAAAGT&alternateBases=NNNNNNNNGT") 
        wrapper.destroy()   
    })
})