<template>
  <div class="content word-wrap">
    ...
    <p>
      ...
      and authorised users. If a user authenticates themselves at ELIXIR AAI,
      their ELIXIR AAI Access Token (JWT) will be saved to cookies for a
      duration of up to one hour. This cookie contains data such as what is the
      user's ELIXIR ID. This cookie is only available from the domain of the
      service, and can only travel in encrypted HTTPS traffic. Other cookies are
      also utilised, but they hold no personal data. Users are advised to delete
      their browser session cookies if they are using the service on a public
      computer.
    </p>
    <p>
      ...
      notification. <br /><b>Cookies for authenticated users:</b> <br /><i
        >oidc_state</i
      >: A session key used in the authentication process at ELIXIR AAI.
      Contains no personal or identifiable data. <br /><i>logged_in</i>:
      Indicates that the user has authenticated. Contains no personal or
      identifiable data. <br /><i>bona_fide</i>: Indicates the user's Bona Fide
      status. Contains no personal or identifiable data. <br /><i
        >access_token</i
      >: ELIXIR AAI JWT access token, is used to gain access to permitted
      datasets at Beacons. Contains user's ELIXIR ID.
    </p>
    <h3>Security</h3>
    <p>
