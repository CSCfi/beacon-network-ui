...
describe('Footer.vue external links', () => {
    ...
    it('"Beacon Project" router-link works', async () => {
        expect(wrapper.find('#beaconProject').attributes().href).toBe("https://beacon-project.io/") 
    })
    it('"ELIXIR Europe" router-link works', async () => {
        expect(wrapper.find('#elixirSite').attributes().href).toBe("https://elixir-europe.org/") 
    })
    it('"GA4GH" router-link works', async () => {
        expect(wrapper.find('#ga4gh').attributes().href).toBe("https://www.ga4gh.org/") 
    })
    it('"GA4GH Discovery" router-link works', async () => {
        expect(wrapper.find('#ga4ghDiscovery').attributes().href).toBe("https://ga4gh-discovery.github.io/") 
    })
